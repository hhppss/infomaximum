...
export const RegisterForm = () => {
    ...
    const addUser = (e) => {
          ...
          if(password!==passwordCurrent){
             return alert.error("Пароли не совпадают")
          }
          else {
            if ((users.map(user => (user.email !== email)))){
              history.push('./login')
            }
            else {
              return alert.error("Пользователь с такой почтой уже существует")
            }
          }
          newUser({
            variables: {
      ...
      <div className="reportContent">
        <FormContainer>
          <Form>
            ...
            <Input value={fname} onChange={e => setFname(e.target.value)} placeholder='Имя'/>
            <Input value={lname} onChange={e => setLname(e.target.value)} placeholder='Фамилия'/>
            <Input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder='Электронная почта'/>
            <div className="password">
              <Input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder='Введите пароль' className="password-input"/>
              <a href="#" className="password-control"> </a>
            </div>
            <div className="passwordCurrent">
              <Input value={passwordCurrent} onChange={e => setPasswordCurrent(e.target.value)} placeholder='Повторите пароль' type="password" name="passwordCurrent" className="password-input-cur"/>
                <a href="#" className="password-control-cur"> </a>
            </div>
            {
              ((fname.length > 0, lname.length > 0, email.length > 0, password.length > 6)) ? <Button onClick={(e) => addUser(e)}>Применить и войти</Button> : <Button onClick={(e) => addUser(e)} disabled="true">Применить и войти</Button>
            }
            
            <LinkText>Уже зарегистрированы? <LinkOnLogin to="/login">Войти</LinkOnLogin></LinkText>
          </Form>
      </FormContainer>
      </div>
    </div>
...
$('body').on('click', '.password-control-cur', function(){
  if ($('.password-input-cur').attr('type') == 'password'){
    $(this).addClass('view');
    $('.password-input-cur').attr('type', 'text');
  } else {
    $(this).removeClass('view');
    $('.password-input-cur').attr('type', 'password');
  }
  return false;
});
